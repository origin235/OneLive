...
- **持久化**: `SharedPreferences` — 键值对本地存储，main() 中预初始化后通过 ProviderScope override 注入
- **Providers**: `lib/features/settings/presentation/providers/settings_providers.dart`
  - `themeModeProvider` — ThemeMode 设置（系统/浅色/深色）
  - `danmakuEnabledProvider` / `danmakuOpacityProvider` / `danmakuFontSizeProvider` / `danmakuSpeedProvider` / `danmakuAreaProvider` — 弹幕参数
  - `qualityLevelProvider` — 默认画质级别
  - `platformEnabledProvider` — StateProvider.family 各平台启禁状态
- **UI**: `SettingsPage` — ConsumerWidget，分组列表式设置页
  - 外观：主题模式（RadioListTile 对话框选择）
  - 播放：默认画质（RadioListTile 对话框选择）
  - 弹幕：开关 + 不透明度/字体大小/速度/显示区域滑块
  - 平台：各平台 SwitchListTile 启禁开关
  - 关于：版本号 (package_info_plus) + 技术栈 + 免责声明
- **主题联动**: `main.dart` 中 `MaterialApp.router` 的 `themeMode` 由 `themeModeProvider` 驱动
- **弹幕联动**: `PlayerPage` 将弹幕参数传入 `DanmakuOverlay`（opacity/fontSize/speed/area）
- **平台过滤**: `PlatformFilterBar` 变为 ConsumerWidget，仅显示已启用的平台
- **Windows 窗口**: `main.dart` 中 window_manager 初始化，最小尺寸 800×480，关闭前保存位置/大小到 SharedPreferences
- **性能优化**: DanmakuOverlay 包裹 `RepaintBoundary`；StreamCard 使用 `ExtendedImage.network` 缓存封面

## 测试与发布 (阶段 9 ✅)

- **测试统计** (2026-04-29):
  - `dart test` (simple_live_core): 43 个测试通过 — LiveRoomItem、LiveAnchorItem、LiveMessage/LiveMessageColor、LiveSuperChatMessage、BinaryReader/BinaryWriter、LiveSite 默认实现、LiveCategory、LivePlayQuality
  - `flutter test` (onelive): 17 个测试通过 — App 冒烟测试、SiteRegistry 注册/查询、Settings Providers 默认值、PlatformFilterBar 渲染/交互、StreamCard 渲染、SettingsPage 渲染/主题对话框

- **构建验证**:
  - `flutter build windows --debug`: 构建成功 → `build/windows/x64/runner/Debug/onelive.exe`
  - Android: `flutter build appbundle --target-platform android-arm64`

- **测试文件结构**:
  - `simple_live_core/test/simple_live_core_test.dart` — Core 模型+接口测试 (43 tests)
  - `onelive/test/unit_test.dart` — SiteRegistry + Settings 单元测试 (10 tests)
  - `onelive/test/widget_test.dart` — Widget 冒烟测试 (7 tests)
