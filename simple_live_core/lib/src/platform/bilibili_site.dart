...
import '../danmaku/bilibili_danmaku.dart';
...
class BiliBiliSite extends LiveSite {
  ...
  Future<LivePlayUrl> getPlayUrls(
    ...
    urls.sort((a, b) => a.contains("mcdn") ? 1 : -1);
    return LivePlayUrl(
      urls: urls,
      headers: {
  ...
  Future<Map> getBuvid() async {
    try {
      if (cookie.contains("buvid3")) {
        return {
          "b_3":
              RegExp(r"buvid3=(.*?);").firstMatch(cookie)?.group(1) ?? "",
          "b_4":
              RegExp(r"buvid4=(.*?);").firstMatch(cookie)?.group(1) ?? "",
        };
      }
      var result = await HttpClient.instance.getJson(
  ...
  String getMixinKey(String origin) =>
      mixinKeyEncTab.fold("", (s, i) => s + origin[i]).substring(0, 32);
  ...
  Future<String> getAccessId() async {
    if (accessId.isNotEmpty) return accessId;
